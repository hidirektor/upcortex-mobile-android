package me.t3sl4.upcortex.UI.Components.RatingBar;
...
public class RatingBar extends View {
    ...
    public enum Gravity {
        ...
        static Gravity fromId(int id) {
            ...
            // default value
            Log.w("RatingBar", String.format("Gravity chosen is neither 'left' nor 'right', I will set it to Left"));
            return Left;
        }
    }
    ...
    private void parseAttrs(AttributeSet attrs) {
        TypedArray arr = getContext().obtainStyledAttributes(attrs, R.styleable.RatingBar);

        borderColor = arr.getColor(R.styleable.RatingBar_srb_borderColor, getResources().getColor(R.color.warningColor));
        fillColor = arr.getColor(R.styleable.RatingBar_srb_fillColor, borderColor);
        starBackgroundColor = arr.getColor(R.styleable.RatingBar_srb_starBackgroundColor, Color.TRANSPARENT);
        backgroundColor = arr.getColor(R.styleable.RatingBar_srb_backgroundColor, Color.TRANSPARENT);

        pressedBorderColor = arr.getColor(R.styleable.RatingBar_srb_pressedBorderColor, borderColor);
        pressedFillColor = arr.getColor(R.styleable.RatingBar_srb_pressedFillColor, fillColor);
        pressedStarBackgroundColor = arr.getColor(R.styleable.RatingBar_srb_pressedStarBackgroundColor, starBackgroundColor);
        pressedBackgroundColor = arr.getColor(R.styleable.RatingBar_srb_pressedBackgroundColor, backgroundColor);

        numberOfStars = arr.getInteger(R.styleable.RatingBar_srb_numberOfStars, 5);

        starsSeparation = arr.getDimensionPixelSize(R.styleable.RatingBar_srb_starsSeparation, (int)valueToPixels(4, Dimension.DP));
        maxStarSize = arr.getDimensionPixelSize(R.styleable.RatingBar_srb_maxStarSize, Integer.MAX_VALUE);
        desiredStarSize = arr.getDimensionPixelSize(R.styleable.RatingBar_srb_starSize, Integer.MAX_VALUE);
        stepSize = arr.getFloat(R.styleable.RatingBar_srb_stepSize, 0.1f);
        starBorderWidth = arr.getFloat(R.styleable.RatingBar_srb_starBorderWidth, 5f);
        starCornerRadius = arr.getFloat(R.styleable.RatingBar_srb_starCornerRadius, 6f);

        rating = normalizeRating(arr.getFloat(R.styleable.RatingBar_srb_rating, 0f));
        isIndicator = arr.getBoolean(R.styleable.RatingBar_srb_isIndicator, false);
        drawBorderEnabled = arr.getBoolean(R.styleable.RatingBar_srb_drawBorderEnabled, true);
        gravity = Gravity.fromId(arr.getInt(R.styleable.RatingBar_srb_gravity, Gravity.Left.id));
    ...
    private void validateAttrs() {
        if (numberOfStars <= 0) {
            throw new IllegalArgumentException(String.format("RatingBar initialized with invalid value for numberOfStars. Found %d, but should be greater than 0", numberOfStars));
        }
        if (desiredStarSize != Integer.MAX_VALUE && maxStarSize != Integer.MAX_VALUE && desiredStarSize
                > maxStarSize) {
            Log.w("RatingBar", String.format("Initialized with conflicting values: starSize is greater than maxStarSize (%f > %f). I will ignore maxStarSize", desiredStarSize, maxStarSize));
        }
        if (stepSize <= 0) {
            throw new IllegalArgumentException(String.format("RatingBar initialized with invalid value for stepSize. Found %f, but should be greater than 0", stepSize));
        }
        if (starBorderWidth <= 0) {
            throw new IllegalArgumentException(String.format("RatingBar initialized with invalid value for starBorderWidth. Found %f, but should be greater than 0",
                    starBorderWidth));
        }
        if (starCornerRadius < 0) {
            throw new IllegalArgumentException(String.format("RatingBar initialized with invalid value for starCornerRadius. Found %f, but should be greater or equal than 0",
                    starBorderWidth));
        }
    }
    ...
    public void setStarSize(float starSize) {
        ...
        if (starSize != Integer.MAX_VALUE && maxStarSize != Integer.MAX_VALUE && starSize > maxStarSize) {
            Log.w("RatingBar", String.format("Initialized with conflicting values: starSize is greater than maxStarSize (%f > %f). I will ignore maxStarSize", starSize, maxStarSize));
        }
        // force re-calculating the layout dimension
        requestLayout();
        generateInternalCanvas(getWidth(), getHeight());
    ...
    private void animateRating(AnimationBuilder builder) {
        ...
        ratingAnimator.addListener(new AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animator) {
                if (ratingListener != null) {
                    ratingListener.onRatingChanged(RatingBar.this, rating, false);
                }
            }
    ...
    private float normalizeRating(float rating) {
        if (rating < 0) {
            Log.w("RatingBar", String.format("Assigned rating is less than 0 (%f < 0), I will set it to exactly 0", rating));
            return 0;
        } else if (rating > numberOfStars) {
            Log.w("RatingBar", String.format("Assigned rating is greater than numberOfStars (%f > %d), I will set it to exactly numberOfStars", rating, numberOfStars));
            return numberOfStars;
        } else {
            return rating;
        }
    ...
    public class AnimationBuilder {
        ...
        private AnimationBuilder(RatingBar ratingBar) {
